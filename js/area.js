...
dsy.add("0_10_11",["崇安区","南长区","北塘区","滨湖区","惠山区","锡山区","新区","江阴市","宜兴市"]);
...
function _init_area(s_province,s_city,k){  //初始化函数
	if(k==1) {
		s1 = [s_province,s_city];
		for(i=0;i<s1.length-1;i++){
			document.getElementById(s1[i]).onchange=new Function("change("+(i+1)+","+k+")");
		}
	} else if(k==2) {
		s2 = [s_province,s_city];
		for(i=0;i<s2.length-1;i++){
			document.getElementById(s2[i]).onchange=new Function("change("+(i+1)+","+k+")");
		}
	} else if(k==3) {
		s3 = [s_province,s_city];
		for(i=0;i<s3.length-1;i++){
			document.getElementById(s3[i]).onchange=new Function("change("+(i+1)+","+k+")");
		}
	} else if(k==4) {
		s4 = [s_province,s_city];
		for(i=0;i<s4.length-1;i++){
			document.getElementById(s4[i]).onchange=new Function("change("+(i+1)+","+k+")");
		}
	}
